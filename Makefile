...
build-image: guard-IMAGE ## Build container image with apko
	...
	docker run --rm -v "${PWD}:/work" -w /work cgr.dev/chainguard/apko:latest build $(IMAGE)/prod.yaml $(IMAGE_NAME):latest $(IMAGE)/$(IMAGE_NAME).tar --arch $(ARCH) --keyring-append /work/keys/melange.rsa.pub --build-repository-append /work/packages
...
build-dev-image: guard-IMAGE ## Build development container image with apko
	...
	@if [ -f "$(IMAGE)/dev.yaml" ]; then \
		docker run --rm -v "${PWD}:/work" -w /work cgr.dev/chainguard/apko:latest build $(IMAGE)/dev.yaml $(IMAGE_NAME):dev $(IMAGE)/$(IMAGE_NAME)-dev.tar --arch $(ARCH) --keyring-append /work/keys/melange.rsa.pub --build-repository-append /work/packages; \
	else \
...
build-shell-image: guard-IMAGE ## Build shell container image with apko
	...
	@if [ -f "$(IMAGE)/shell.yaml" ]; then \
		docker run --rm -v "${PWD}:/work" -w /work cgr.dev/chainguard/apko:latest build $(IMAGE)/shell.yaml $(IMAGE_NAME):shell $(IMAGE)/$(IMAGE_NAME)-shell.tar --arch $(ARCH) --keyring-append /work/keys/melange.rsa.pub --build-repository-append /work/packages; \
	else \
